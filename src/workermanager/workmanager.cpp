...
//     if(this->m_FileIsEmpty) cout << "文件不存在或文件为空！" << endl;
//     else{
...
WorkerManager::WorkerManager() {
	...
	char ch;
	ifs >> ch;//只读一个字符
	if (ifs.eof()) { //eof() :end of file 返回bool
		//cout << "文件为空！" << endl;
		this->m_EmpNum = 0;
		this->m_EmpArray = NULL;
		this->m_FileIsEmpty = true;
...
void WorkerManager::Find_Emp() {
	if (this->m_FileIsEmpty) {
		...
		case 2: {//按照姓名查找
			...
			cout << "请输入职工姓名：" << endl;
			cin >> name;
			bool flag = false;//找到标记
			for (int i = 0; i < this->m_EmpNum; i++) {
				if (this->m_EmpArray[i]->m_Name == name) {
					cout << "查找成功！该职工职工号为："<< this->m_EmpArray[i]->m_Id <<"  信息为：" << endl;
					this->m_EmpArray[i]->ShowInfo();
					flag = true;
				}
			}
